// Generated on 2016-06-05 using generator-ionic 0.8.0
'use strict';
...
module.exports = function (grunt) {
  ...
  grunt.initConfig({
    ...
    ngconstant: {
      options: {
        ...
        wrap: '"use strict";\n\n {%= __ngModule %}',
        name: 'config',
        dest: '<%= trip.app %>/<%= trip.scripts %>/configuration.js'
      },
      development: {
        constants: {
          ENV: {
    ...
    watch: {
      ...
      js: {
        files: ['<%= trip.app %>/<%= trip.scripts %>/**/*.js'],
        tasks: ['newer:copy:app', 'newer:jshint:all']
      },
      styles: {
        files: ['<%= trip.app %>/<%= trip.styles %>/**/*.css'],
    ...
    autoprefixer: {
      ...
      dist: {
        files: [{
          expand: true,
          cwd: '.temp/<%= trip.styles %>/',
          src: '{,*/}*.css',
          dest: '.temp/<%= trip.styles %>/'
        }]
      }
    },
    ...
    usemin: {
      html: ['<%= trip.dist %>/**/*.html'],
      css: ['<%= trip.dist %>/<%= trip.styles %>/**/*.css'],
      options: {
        assetsDirs: ['<%= trip.dist %>']
      }
    },
    ...
    copy: {
      dist: {
        files: [{
          ...
          dest: '<%= trip.dist %>',
          src: [
            '<%= trip.images %>/**/*.{png,jpg,jpeg,gif,webp,svg}',
            '*.html',
            'templates/**/*.html',
            'fonts/*'
          ]
          ...
          expand: true,
          cwd: '.temp/<%= trip.images %>',
          dest: '<%= trip.dist %>/<%= trip.images %>',
          src: ['generated/*']
        }]
      },
      styles: {
        expand: true,
        cwd: '<%= trip.app %>/<%= trip.styles %>',
        dest: '.temp/<%= trip.styles %>/',
        src: '{,*/}*.css'
      },
      fonts: {
    ...
    //   dist: {
    //     files: {
    //       '<%= trip.dist %>/<%= trip.styles %>/main.css': [
    //         '.temp/<%= trip.styles %>/**/*.css',
    //         '<%= trip.app %>/<%= trip.styles %>/**/*.css'
    //       ]
    //     }
    //   }
    // },
    ...
    //   dist: {
    //     files: {
    //       '<%= trip.dist %>/<%= trip.scripts %>/scripts.js': [
    //         '<%= trip.dist %>/<%= trip.scripts %>/scripts.js'
    //       ]
    //     }
    //   }
    ...
    karma: {
      options: {
        ...
        files: [
          '<%= trip.app %>/bower_components/angular/angular.js',
          '<%= trip.app %>/bower_components/angular-mocks/angular-mocks.js',
          '<%= trip.app %>/bower_components/angular-animate/angular-animate.js',
          '<%= trip.app %>/bower_components/angular-sanitize/angular-sanitize.js',
          '<%= trip.app %>/bower_components/angular-ui-router/release/angular-ui-router.js',
          '<%= trip.app %>/bower_components/ionic/release/js/ionic.js',
          '<%= trip.app %>/bower_components/ionic/release/js/ionic-angular.js',
          '<%= trip.app %>/<%= trip.scripts %>/**/*.js',
          '<%= trip.test %>/mock/**/*.js',
          '<%= trip.test %>/spec/**/*.js'
        ],
        ...
        singleRun: false,
        preprocessors: {
          // Update this if you change the trip config path
          '<%= trip.app %>/<%= trip.scripts %>/**/*.js': ['coverage']
        },
        coverageReporter: {
          reporters: [
    ...
    ngAnnotate: {
      dist: {
        files: [{
          expand: true,
          cwd: '.temp/concat/<%= trip.scripts %>',
          src: '*.js',
          dest: '.temp/concat/<%= trip.scripts %>'
        }]
      }
    }
  ...
  // over to <%= trip.dist %>/. Last step is running cordova prepare so we can refresh the ripple
  ...
  grunt.registerTask('watch:karma', function () {
    var karma = {
      files: ['<%= trip.app %>/<%= trip.scripts %>/**/*.js', '<%= trip.test %>/spec/**/*.js'],
